...
\noindent Alternatively the user can provide her/his own gene-disease association file. In order to transform that input file in a \texttt{molecularComorbidity} class object the \texttt{dataframe2molecularcomorbidity} function.\\
