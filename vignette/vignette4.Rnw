...
\graphicspath{{G:/comorbidity/Data/} }
\hypersetup{
    colorlinks,
    citecolor=blue,
    filecolor=blue,
...
\author{Alba Gutiérrez-Sacristán \and Àlex Bravo \and Alexia Giannoula \and Miguel A. Mayer \and  Ferran Sanz \and Laura I. Furlong}
...
\begin{enumerate}
\item Age and sex analysis of the population diagnosed with the disease of interest. 
\item Clinical comorbidity analysis, based on diagnosis data, in a specific sex and age interval. 
...
\item \textbf{patient\_sex}: the patient sex is required to perform a comorbidity analysis stratified by sex and for the sex ratio analysis. It can be numeric (\textit{e.g., 0 for one sex and 1 for the other}), character (\textit{e.g., M for male and F for female}) or a list of characters (\textit{e.g., male and female}).
...
\item A boxplot showing the age distribution by sex. 
\item A pie chart representing the sex distribution.  
\end{itemize}
...
@
\end{center}
\caption{Summary plot containing: age distribution, age distribution by sex and sex distribution.} 
\label{fig:summaryDB}
\end{figure}
...
\noindent The output of the \texttt{diseasePrevalence} function is a barplot showing the disease prevalence in the entire population and the disease prevalence according to sex (Figure \ref{fig:diseasePrevalence}):
...
@
\end{center}
\caption{Barplot showing the disease prevalence in all the population and also, according to the sex. } 
...
\item \texttt{sex}: determine what is the sex of interest for performing the comorbidity analysis. 
\end{itemize}
...
\noindent The \texttt{comoRbidity} package also estimates the sex ratio (SR) parameter. The sex ratio (SR) parameter allows to see if a comorbidity suffered in both, men and women, is equally likely in both sex or if it is more likely in one sex than in another. For a comorbidity A and age group t, SR \eqref{eq:solve} is defined by Klimek et al. \cite{REF15} as follows:\\
...
\begin{enumerate}
\item Apply the \texttt{sexRatio} function using as input the \texttt{cAnalysis} object obtained for both sex. 
...
\item \textbf{\texttt{sex}}: determines what is the sex of interest for performing the directionality analysis. 
...
\noindent As a result a \texttt{data.frame} is obtained. This \texttt{data.frame} contains 7 columns, with the comorbidity disorders and the directionality results in numeric and character format.\\
...
<<exampleData2_codesMolecular>>=
filePth <- system.file("extdata", package="comoRbidity")
genediseaseData <- read.delim( file   = paste0(filePth, "/genediseaseTable.txt"), 
                               header = TRUE, 
                               sep    = "\t" )
head(genediseaseData)
@

\colorbox{beaublue}{\begin{minipage}{14cm}
\noindent \textbf{Note that if you use this format data for the molecular comorbidity analysis, the overview of the gene-disease data performed applying the \texttt{summaryDiseases} will not be possible}\\
\end{minipage}}
\\
...
\subsubsection{Using as input a list of diseases}
...
\subsubsection{Using as input a list of gene-disease associations}

\noindent Alternatively the user can provide her/his own gene-disease association file. In order to transform that input file in a \texttt{molecularComorbidity} class object the \texttt{dataframe2molecularcomorbidity} function. The input file must contain at least two predefined columns named as stated below:
\begin{itemize}
\item \textbf{gene}: a gene identifier, that can be numeric, alphanumeric or a list of characters.
\item \textbf{diseaseName}: a disease identifier, that can be numeric, alphanumeric or a list of characters.
\\

\noindent If the input file does not contain the required columns the next message will appear:
<<messageInputMolecular, echo = FALSE>>=
message("Check the input file structure. Remember that this
                file must contain at least two columns with the column
                    names as follows:\n -> gene \n -> diseaseName\n")
@

<<geneDiseasefileExample, message=FALSE, warning=FALSE>>=
geneDiseaseFile <- read.delim(system.file("extdata", "genediseaseTable.txt", package="comoRbidity"))
head( geneDiseaseFile )
@
\\

\noindent As input the function only requires one argument, \textbf{\texttt{filePth}}, that determines the file name with the complete path where the file with gene and diseases of interest is located. There is not restriction about the gene and/or disease identifier class. However, the user should take into account that the \texttt{summaryDiseases} function cannot be applied. Nevertheless, the molecular comorbidity analysis can be applied. 

<<mcUserManual, message=FALSE, warning=FALSE>>=
genediseaseFile <- paste0(filePth, "/genediseaseTable.txt")
manualmc <- dataframe2molecularcomorbidity( filePth = genediseaseFile)
manualmc
@
\\
...
\colorbox{beaublue}{\begin{minipage}{14cm}
\noindent \textbf{Note that if you apply this function to a \texttt{molecularComorbidity} object obtained applying the \texttt{dataframe2molecularcomorbidity} function, the next error message will appear: \texttt{Sorry, this function is only available for object obtained after applying the \texttt{queryMolecular} function.}}\\
\end{minipage}}
\\
