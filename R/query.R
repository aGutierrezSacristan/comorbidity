...
query <- function( databasePth, codesPth, birthDataSep, admissionDataSep, intraCodes = FALSE, aggregatedDis = FALSE, python= FALSE, verbose = FALSE, warnings= TRUE) {
    ...
    if( python == TRUE ){
        ...
        final$age <- as.numeric(final$admissionStartDate) - as.numeric(final$patient_dateBirth)
        all$age <- as.numeric(all$admissionStartDate) - as.numeric(all$patient_dateBirth)
        direct$age <- as.numeric(direct$admissionStartDate) - as.numeric(direct$patient_dateBirth)
    }
